...
const express = require('express');
const multer = require('multer');
const { verifyClaim, validateOtp, uploadClaim } = require('../controller/claimController');

const router = express.Router();
const upload = multer({ dest: "uploads/" });

router.post("/claim-restaurant/verify", verifyClaim);
router.post("/claim-restaurant/validate-otp", validateOtp);
router.post("/claim-restaurant/document", upload.single("document"), uploadClaim);
module.exports = router;  
